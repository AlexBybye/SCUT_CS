...
1. 已知用梯形法求积分的公式为：![formula-object](assets/cpp-039/image-001.png)，其中，*h*  = (*b**a*) /  *n*，*n*为积分区间的等分数，n=10000编程求如下积分的值。要求：把求积分公式编写成一个函数，并使用函数指针作为形式参数。调用该函数时，给出不同的被积函数作为实际参数求不同的积分。

① ![formula-object](assets/cpp-039/image-002.png)      ② ![formula-object](assets/cpp-039/image-003.png)      ③ ![formula-object](assets/cpp-039/image-004.png)
