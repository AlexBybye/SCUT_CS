---
source_id: engineering-mathematical-analysis-1-006
course_id: engineering_math_analysis_1
title: "2023级工科数学分析（一） 期末考试题(A卷)"
original_file: "学科资料/工科数学分析I/历年试卷/23A，23B及答案/2023级工科数学分析（一）   期末考试题(A卷).pdf"
document_role: past_exam
year: 2023
locator_type: page
---

# 2023级工科数学分析（一） 期末考试题(A卷)

<!-- page: 1 -->

… … … … … … … … … … … … … … … … … … 密… … … … … … … … … … … … … … … … … … 封… … … … … … … … … … … … … … … 线… … … … … … … … … … … … … …

姓名                学号
  学院                  专业班级                     座位号

诚信应考，考试作弊将带来严重后果！

 华南理工大学本科生期末考试

《工科数学分析（一）》2023—2024 学年第一学期期末考试试卷（A）卷

注意事项：1. 开考前请将密封线内各项信息填写清楚；
          2. 所有答案请直接答在试卷上；
          3．考试形式：闭卷

          4. 本试卷共 5 个  大题，满分100 分， 考试时间120 分钟。

题 号
一
二
三
四
五
总分
得 分

                                            评阅教师请在试卷袋上评阅栏签名

得分

<!-- question: engineering-mathematical-analysis-1-006-Q1 -->

一、填空题（共5 小题，每小题3 分，共15 分）

!→# 𝑓(𝑥) = 𝐴 的 𝜀−𝛿 定义是

１. lim

                                                             ；
２. 设 𝑦= cos\$ 𝑥，则 d(\$&\$')𝑦=

( 密 封 线 内 不 答 题 )

_____________ ________

                                                             ；

)

３. 求曲线 𝑟= 2(1 + cos 𝜃) 在 𝜃=

* 处的切线方程

                                                               ；

４. 已知 𝑓(𝑥) = ∫𝑡! ln 𝑡d𝑡,   𝑥> 0,

+
&
 则 𝑓(𝑥) 的表达式为                  ；

)

５. 函数 𝑦= 𝑥+ 2 sin 𝑥 在 >

\$ , 𝜋@上的最大值为              .

得分

<!-- question: engineering-mathematical-analysis-1-006-Q2 -->

二、计算下列各题（共3 小题，每小题8 分，共24 分）

,-(+.!.!!).,-(+/!.!!)

1. 求极限 lim
!→&

012 !/230 !
.

《  工科数学分析（一）  》试卷 第 1 页 共 6 页

<!-- page: 2 -->

2.  求不定积分 ∫A
!/+

\$/! d𝑥, 1 < 𝑥< 2.

3.  已知 𝑓(𝑥) = ∫
!!/4!

!!.4!
+
&
d𝑦, 求 ∫𝑓(𝑥)

+
&
d𝑥.

《  工科数学分析（一）  》试卷 第 2 页 共 6 页

<!-- page: 3 -->

得分

<!-- question: engineering-mathematical-analysis-1-006-Q3 -->

三、解答下列各题（共4 小题，每小题8 分，共32 分）

1. 设 𝑥+ = 10, 𝑥5.+ = C6 + 𝑥5 (𝑛= 1,2, ⋯), 请问数列 {𝑥5} 的极限是否存在？若存在请计算
这个极限。

2. 设 𝑓(𝑥) 在点 𝑥= 0 的邻域内二阶可导，且

\$%& !'!((!)

                                         lim

!!
= 0.

!→#

求 𝑓(0), 𝑓+(0), 𝑓++(0) 的值。

《  工科数学分析（一）  》试卷 第 3 页 共 6 页

<!-- page: 4 -->

3. 试说明摆线 I𝑥= 𝑎(𝑡−sin 𝑡);
𝑦= 𝑎(1 −cos 𝑡), 𝑡∈[0,2𝜋] 所确定的函数 𝑦= 𝑦(𝑥) 是凹函数。

"
#
&
 的全长，其中 𝑥∈[0, 𝑛𝜋], 𝑛 为正整数。

4.  求曲线 𝑦= ∫𝑛√sin 𝑡d𝑡

《  工科数学分析（一）  》试卷 第 4 页 共 6 页

<!-- page: 5 -->

得分

<!-- question: engineering-mathematical-analysis-1-006-Q4 -->

四、证明题（共2 小题，每小题10 分，共20 分）

5.+ < ln P1 +
+

+

5Q <
+

5 , 𝑛∈ℕ..

      1.  证明不等式

(
)
+
-¥
¥
，

2.  证明：函数 𝑓(𝑥) = e/!! 在
上一致连续.

《  工科数学分析（一）  》试卷 第 5 页 共 6 页

<!-- page: 6 -->

得分

<!-- question: engineering-mathematical-analysis-1-006-Q5 -->

五、应用题（本题9 分）

       一抛物线 𝑥= 𝑎𝑦\$ + 𝑏𝑦+ 𝑐 的轴平行于 𝑥 轴，开口向左且通过原点以及 (2,1).

求当它与 𝑦 轴之间的面积为最小时的抛物线方程。

《  工科数学分析（一）  》试卷 第 6 页 共 6 页
