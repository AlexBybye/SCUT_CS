...
> 1.7 修订：可靠替换为 LaTeX 且完成引用检查后，同批删除已无引用的公式资源；原始文档或 Git 版本作为回退依据，不在 `knowledge/` 长期保留无引用公式图片。1.6 的公式展示规则、1.5 的轻量去重、1.4 的排除项与轻量验证、1.3 的全链路身份脱敏门禁继续生效。
...
- 插件只能处理复制到 `docs/review_artifacts/<source_id>/staging/` 的副本，禁止直接对 `学科资料/` 原目录执行单文件或批量转换；
...
- 插件把公式、变量或特殊数学符号导出成图片时，该图片只用于对照和失败回退，不是首选最终展示；规范化阶段应按 4.2 转为 LaTeX；
- 插件不得改变隐私门槛：staging 仅限本地审核，任何含未脱敏学生／贡献者身份信息的粗稿、图片或调试 HTML 均不得进入 `knowledge/`、公开分支或 PR。

### 4.2 公式与特殊数学符号

规范 Markdown 按以下规则处理数学表达：

- 夹在正文中的变量、希腊字母、上下标和短公式使用行内 `$...$`，例如 `随机变量 $X$ 服从 $N(0,1)$`；
- 单独成行、分式较长、含多层上下标、矩阵、分段函数或推导链的公式使用独立公式块，定界符单独占行：
...
- 不用 PNG、JPG、WMF 或截图展示本可可靠表达的纯公式和特殊数学符号；也不把正文中的单个变量保留成碎片图片；
- 转换依据的优先级为：原生公式结构（如 OMML）→ 可确定解析的公式对象（如 MathType/MTEF）→ 对照原件确认后的人工转写。数学 OCR 只生成候选，不能仅凭 OCR 结果批量替换；
- 坐标图、受力图、电路图、几何图、流程／结构图、知识性插图、整页扫描和手写页不是公式图片，继续保留到稳定 `assets/`；图中标注不要求拆成零散 LaTeX；
- 无法可靠还原的公式继续保留原公式图片并在 `notes` 标明，状态保持 `pending` 或进入 `needs_fix`；不得依据上下文、答案或常识猜写 LaTeX；
- 公式图片被可靠替换后，在同一批次完成公式配对、链接存在性和全库引用检查；确认当前 Markdown 已不再引用且资产不包含图形知识时，删除相应资源，不在 `knowledge/` 长期保留无引用公式图片。人工复核前以原始 DOCX/PDF 或 Git 版本作为回退依据；不得误删同一对象中的坐标轴、示意图或其他非公式内容；
- 已经 `passed` 的资料发生公式图片→LaTeX 重写后，原审核只覆盖旧生成物；该文件应回到 `pending`，待人工核对新公式后再恢复 `passed`。
...
1. 在文件名和原始路径完成身份脱敏后，把允许处理的 DOCX 复制到 `docs/review_artifacts/<source_id>/staging/`；旧 DOC 则先在这里生成 `converted.docx` 和 `rendered.pdf`；
2. 只对 staging 中的 DOCX 使用“转换为Markdown”；不在原资料目录使用插件默认的同目录输出；
3. 将插件输出视为 `rough.md`，同时保留本次审核实际用到的调试 HTML 和临时图片，但它们不得进入 `knowledge/`；
4. 对照原件恢复正确阅读顺序、标题、列表、公式、表格、代码和图片；公式与特殊数学符号按 4.2 转为 LaTeX，不根据常识补写；
5. 把需要保留的非公式知识图片，以及暂时无法可靠还原的公式图片，迁移到 `knowledge/<course_id>/assets/<source_id>/`，使用不含学生／贡献者身份且稳定的文件名，并重写 Markdown 引用；
...
公式 OCR 不直接套用普通文字 OCR 的通过判断。即使公式 OCR 分数较高，也必须能与原生公式结构、MathType/MTEF 对象或原件视觉内容逐项对应；否则只保留为候选，不写入规范 Markdown。
...
发生公式图片→LaTeX 改写时，只做一次轻量批次检查：检查 `$`／`$$` 是否配对、独立公式定界符是否单独占行、Markdown 中是否仍引用已删除资产、以及残余图片是否属于图表／扫描页或已记录的不确定公式。除工具告警位置和代表性公式外，不为同一未变化内容重复逐页验证。
...
`docs/review_artifacts` 只保存实际生成并用于审核的中间产物，不要求为不存在的产物创建空文件。`staging/` 是本地隔离区，不进入 Git、公开分支或 PR；确需公开提交的 review artifact 必须使用不含学生／贡献者姓名、班级、学号的路径，并先检查其可见内容和文档元数据已经完成同类脱敏。
