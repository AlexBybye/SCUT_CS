...
>
> 1.8 修订（2026-08-23）：新增插入期「迭代 7.5」（§12A）。2026-08-23 对迭代 0～6 退出记录与本 SOP §3～§11 的只读审计确认：各期功能主链路均已按退出条件闭合且未伪造通过，但存在三类欠账——外部证据长期挂起、运维与账号生命周期能力缺位、知识库人工审核瓶颈导致课程包过期。迭代 7.5 不改产品功能语义，专门清偿技术债、补齐验收证据并重建课程包；完成或显式挂起后方可进入后续期。
>
> 1.9 修订（2026-08-23）：迭代 7.5 收尾后经维护者决议——迭代 8（§13）改为证据触发的条件期，新增量化进入条件与「首切片／后置」范围切分；迭代 9（§14）拆分处置：视觉索引与图片相似检索关闭，全仓扩展附重启条件暂缓。1.8 修订中「完成或显式挂起后方可进入后续期」的后续期指向自本版起以 §18.2 执行顺序为准，不再默认是迭代 8。`docs/PLAN-1.md` 中迭代 8／9 的原始路线条目由本版起以本 SOP 为准。
...
- [ ] 本节全部条目处理完毕后更新 §18 启动清单，再按 §18.2 执行顺序分流（2026-08-23 决议后不再默认进入迭代 8）。
...
2026-08-23 维护者决议：本迭代不再是迭代 7.5 收尾后的默认下一期，改为满足 13.1.1 全部
进入条件后才启动的证据触发条件期。理由：应用尚无生产部署与真实用户，学生可见功能的价值
无法度量；文本主链路评测欠账未收敛；资料轨道已有 387 张唯一公式图视觉转写弃权的反面证据；
平台免费额度通道（OpenRouter 每分钟 20 次、项目 credits 少于 10 美元时每日 50 次，全体共享）
承受不了图片 token 消耗。方向保留，由证据启动。

#### 13.1.1 进入条件（全部满足才允许进入实施）

- [ ] 前置义务已清：迭代 7.5 分组 C 的真实模型评测收敛，分组 D 的课程包重建、激活与回退演练完成；
- [ ] 视觉能力探针通过：弃权队列 387 张唯一公式图换 glm-4.6V 按既有三道闸重跑一次，
      三道闸全通过比例不低于 85%，且按 44 个已转写文件同口径人工抽查 LaTeX 语义错误率不高于 5%；
      两个阈值为本次决议预设值，探针完成后、实施进入前由维护者按实测分布一次性确认或收紧，调整须记录理由；
- [ ] 真实需求证据存在：应用有可访问部署形态，且有真实用户提出的图片提问需求记录
      （反馈、issue 或使用数据），不接受纯推测需求；
- [ ] 模型与额度口径已书面决议：默认仅平台免费视觉模型可用且 feature flag 默认关闭；
      BYOK 新增视觉模型属于 §3.3 固定目录变更，必须单独立决策门，不随本期默认带入。

### 13.2 执行动作

标注口径：【首切片】＝条件达成进入后先做且只做；【后置】＝重新排队，不在首切片退出前实施。

- [ ] 【首切片】实现图片上传和临时存储；
- [ ] 【首切片】限制格式、大小、像素和频率；
- [ ] 【首切片】实现 OCR；
- [ ] 【首切片】OCR 低于 `0.85` 时进入用户确认、裁剪或重传路径；视觉模型没有原生校准置信分，
      “OCR 置信度”落地为资料轨道三道闸（三票多数决 → 确定性校验 → mathtext 渲染闸）
      的等价确定性规则并在契约测试固化，不得伪造数值来源；
- [ ] 【首切片】高于 `0.85` 也不自动宣称识别正确；
- [ ] 【首切片】支持公式识别与 LaTeX；
- [ ] 【后置】支持表格、图表、流程图和结构图解释（首切片只要求转写级保真）；
- [ ] 【后置】支持图片题目知识点识别和分级提示；
- [ ] 【后置】支持用户图片作答讲评；
- [ ] 【首切片】区分用户上传、仓库资料和通用知识来源；
- [ ] 【首切片】原始图片、OCR 中间结果和附件执行 7 天 TTL，
      复用 `cleanup_material_records()` 与周期清理调度器；
- [ ] 【首切片】30 天历史只保留回答、来源元数据和附件过期标记。

### 13.3 必验场景

- 低置信判定为确定性规则，同一图片重跑结论一致；
- OCR 低置信不静默猜测；
...
首切片范围（图片上传与限制、OCR 及确定性低置信交互、公式 LaTeX 转写、来源区分、7 天 TTL
清理）通过固定测试即视为本期退出；【后置】项不进入首切片退出条件，重新排队时另立进入记录。
视觉相似检索继续保持关闭。
...
原目标：在已有文字检索基线上验证视觉检索是否真的增益，并把构建能力扩展到约 10GB 全仓资料。

2026-08-23 维护者决议：本期不再作为默认后续期，拆分处置如下；14.2～14.4 条目原文保留，
仅在对应半区重启时生效：

- 视觉检索半区（页面／幻灯片视觉索引、图片与仓库页面相似检索及其对照评测）：**关闭，不再排期**。
  该半区自带“只有明显优于纯 OCR 文本检索才开放学生端”的对照门槛，大概率投入整期基建后得到
  不开放的结论；重启需要新的书面决议并引用新的能力或需求证据。
- 全仓扩展半区（约 10GB 增量构建）：**暂缓，附重启条件**。当前瓶颈是知识库人工审核吞吐
  （1705 行待审基线、45／55 门注册课无课程包产物），先扩构建能力只会放大积压。
  重启条件为预设值，调整须记录理由：55 门注册课程中不少于 45 门已有 active 课程包，
  且 manifest 待审行少于 200。
...
- [ ] 【已关闭】建立纯 OCR 文本检索对照；
- [ ] 【已关闭】用固定测试比较两条路径；
- [ ] 【已关闭】只有视觉方案明显更好时才开放学生端；
- [ ] 【暂缓】全仓只消费 GitHub 主分支中 `passed` 内容；
- [ ] 【暂缓】全仓扩展继续按固定 commit 和必要路径增量获取内容，不让 App CI 或 Docker 构建携带全部资料；
- [ ] 【暂缓】支持约 10GB 资料的增量 candidate 构建；
- [ ] 【暂缓】建立缓存、性能和逐课程质量收敛；
- [ ] 【暂缓】保留课程开关、active 回退和构建失败保护；
- [ ] 【暂缓】跨课程继续由 feature flag 控制并默认关闭。
...
- [ ] 确认 PLAN-1 v1.11 和本 SOP v1.9 仍是最新版本
...
KaTeX 路由级按需加载。进入迭代 8 前的分流（期次指向已由下方 §18.2 决议更新）：
...
### 18.2 迭代 8／9 处置决议与迭代 7.5 后的执行顺序（2026-08-23）

经维护者确认：迭代 8 改为证据触发的条件期（§13.1.1），迭代 9 拆分处置（§14.1）。
迭代 7.5 收尾后的执行顺序更新为：

```markdown
- [ ] 清偿 7.5 分组 C／D：SCUT 评测集真实模型收敛，课程包重建、激活与回退演练按退出记录指令执行
- [ ] 安全债：OpenRouter 控制台撤销／轮换旧 Key，补控制台操作时间记录
- [ ] 部署推进：评估把原定迭代 10 的 SWR→ECS 提前，或在预算获批前以受限灰度
      （如既有 Tailscale 隧道验收口径）取得真实使用者与需求记录；
      预算未获批期间 DEPLOYMENT_ENABLED 保持未设置或 false，不违反 §3.6 fail-closed
- [ ] 执行 387 张公式图 glm-4.6V 重跑探针（同时销 §12A 分组 D 挂起项），
      结果按 §13.1.1 进入条件判定是否启动迭代 8 首切片
- [ ] 迭代 9 不再是默认后续期：视觉半区已关闭，全仓扩展按 §14.1 重启条件另行评估
```

本清单只改变期次顺序，不改变 §12A 各挂起项的销账要求；§15 验收矩阵对条件期同样适用。
历史状态文档（含 `ITERATION_7_STATUS.md` 中“下一期为迭代 8”等当时事实表述）按原样保留，
期次冲突以本节为准。
