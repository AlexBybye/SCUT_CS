...
- [ ] Trace 排序分数学生端展示程度形成配置决策与默认值（销 §16 待确认项 1）;
- [ ] KaTeX 等重依赖按需加载，重新执行性能审计并更正 README／历史记录中的体积结论。

分组 C——维护者视图与评测收敛：

- [ ] 补齐 §9.2 维护者轻量视图缺口：资料处理状态、反馈筛选、高频失败列表、同题重跑入口、当前语料／课程包版本、逐课程评测结果；
- [ ] 收敛 SCUT 评测集：真实 corpus 与真实模型下处理 6 个长期失败的 `knowledge_qa` case——转绿，或在 case 内注明阻塞原因后调整期望；不允许无限期红着也不允许静默删除；
- [ ] 在激活后的 corpus 上执行 exam_review 双路径 ×10 门逐课程评测并留存报告（补迭代 5 未执行的退出条件）；
- [ ] 跨课程开放门槛形成决策（销 §16 待确认项 4）：最大同时课程数、开启前提示文案、配置默认值。
...
- [ ] 加密 zip：密码到位后转换入库，或书面决议不入库；【外部输入】
- [ ] 视觉转写弃权队列（387 张唯一公式图）换更强视觉模型重跑一次；44 个已转写文件对照原件抽查 LaTeX 正确性并记录结论。
...
进入实现前确认：周期调度器的形态（进程内线程、系统 cron、还是独立 worker 任务）与其在单机部署下的失效语义；账号注销对 GitHub 身份映射与贡献记录的处理口径（匿名化还是物理删除）；KaTeX 按需加载的粒度（路由级还是组件级）。未确认前对应条目保持禁用态或本地实现，不得冒充生产就绪。
