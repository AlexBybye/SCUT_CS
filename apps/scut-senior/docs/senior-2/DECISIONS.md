...
- **决策**：阶段二事件在 SQLite 中按 run 追加保存，并同步保存 reducer 派生快照；快照写入必须通过事件重放校验。取消、失败、预算耗尽和成功终态都归入同一事件链。
- **兼容**：对外 NDJSON 增加可选 `agent` 事件，默认关闭；开启
  `SCUT_SENIOR_AGENT_EVENT_STREAM_ENABLED` 后，旧客户端仍可继续消费原有
  `trace / answer_delta / result / error` 事件。
- **边界**：事件只服务于单个 run 的状态闭环，不改变学生侧答案契约，不引入独立 Planner、ReAct 框架或消息队列。
