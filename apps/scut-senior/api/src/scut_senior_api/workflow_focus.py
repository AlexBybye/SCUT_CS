...
_TONE_VISIBLE_CALLOUTS = {
    Tone.TEACHING_ASSISTANT: (
        "> **助教提示：** 思路要清晰，依据要扎实——推导过程比答案本身更重要。"
    ),
    Tone.SENIOR_STUDENT: (
        "> **学长提醒：** 哥们儿别急，静下心来多想一步，答案往往藏在细节里。"
    ),
    Tone.STUDY_PARTNER: (
        "> **复习搭子提醒：** 看懂不等于学会哦～自己过一遍，印象才更深刻！"
    ),
}
...
_TONE_DIRECTIVES = PERSONA_PROFILES
...
def build_workflow_focus(request: WorkflowRunRequest) -> WorkflowFocus:
    ...
    if request.workflow_type == WorkflowType.KNOWLEDGE_QA:
        ...
        directive = (
            ...
            "不得自行编造或改写统计数字。"
            "不要在回答中重新粘贴完整用户大纲，也不要重复系统附录中的完整统计；"
            "请把篇幅用于解释复习顺序、具体易错点和可执行的练习方式。"
