...
- 检索相关性分数地板（2026-08-23 落地）：`SCUT_SENIOR_RETRIEVAL_MIN_SCORE`（默认 6）为 local_corpus 检索的加权重叠下限，单一中文 bigram 碰撞（仅 2 分）这类噪声候选不再进入引用 Guard；无候选过线的查询诚实返回"证据不足"。此前弱锚点查询的真实模型评测翻转即源于此缺口，收敛集 `_note_7` 登记的改进候选就此销账；
