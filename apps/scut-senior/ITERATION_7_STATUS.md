...
## 收尾决议（2026-08-23，使用者逐项拍板）

1. **README 标注贡献面板为维护者内部功能**：已写入"明确关闭或待确认"节
   （后端完整、前端 `CONTRIBUTION_SUBMIT_CLOSED=true` 封闭，开放即改回 false）。
2. **manifest 余量 4 行纯图文件批准通过**：电工学原卷答案／原卷·机械、java 往年试题、
   移动应用开发清朝试卷转 `passed`——按整页图预览保留、不产出文本 chunk（迭代 8 关闭
   口径），active 语料内容不变，下次重建自然吸收。
3. **检索相关性分数地板落地**：`SCUT_SENIOR_RETRIEVAL_MIN_SCORE`（默认 6）+
   两个被它暴露的配套缺口修复——服务层 `retrieval_context_carry` 追问轮补锚重试
   （Trace 留痕）、exam_review 无大纲路径以计划引用的试卷标题作检索锚点。
   测试 **576 passed**（新增 6 个：地板行为×2、越界校验、上下文组合、试卷锚点、
   双轮集成）。真实模型收敛集末两轮 10/1/1 与 9/2/1：`insufficient-evidence-001`
   与 `multi-turn-followup-001R` 确定性转绿；其余两例翻转互不相同且本地复算检索
   均有候选（raw≥6），判定为上游免费档网关回退方差，两例修复后均至少一轮全绿。
