...
[AlexBybye/subtask_skill](https://github.com/AlexBybye/subtask_skill) · `SKILL.md` 位于仓库根目录

面向复杂任务的任务编排 Skill。先判断任务是否包含至少两个真正独立、且并行收益高于派发与合成成本的工作流，再按责任边界拆解子任务、建立依赖关系、并行派发 subagent，最后统一合成并验证结果。适用于多模块实现、迁移、批处理、多来源研究和并行调查等场景。

**不做什么**：不用于单文件小改动、强串行调试、范围尚未明确的探索、共享契约或可变状态的修改，也不会因为任务步骤多或文件多就强行拆分。默认控制在 3 到 8 个子任务、单层派发，每个失败子任务最多重试一次。

**适合谁**：经常处理 API、UI、数据库、测试和文档等多条工作流，想让复杂任务先明确分工、依赖和验收方式，再交给多个 subagent 协作完成的同学。
